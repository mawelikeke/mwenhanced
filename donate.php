<?php
// Including the needed files for this to work
...
$notify_email =  (string)$MW->getConfig->donation->PayPal_Email;  //email address to which debug emails are sent to
$DB_Server =  "" . $MW->getDbInfo['db_host'] . ":" . $MW->getDbInfo['db_port'] . ""; //your MySQL Server
$DB_Username = (string)$MW->getDbInfo['db_username']; //your MySQL User Name
$DB_Password = (string)$MW->getDbInfo['db_password']; //your MySQL Password
$DB_DBName = (string)$MW->getDbInfo['db_name']; //your MySQL Database Name

if (!$fp) {
// HTTP ERROR
echo "Could not connect to Paypal";
} else {
fputs ($fp, $header . $req);
while (!feof($fp)) {
$res = fgets ($fp, 1024);
