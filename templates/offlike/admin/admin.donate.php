...
if (isset($_POST['donate_username']) && isset($_POST['donate_items'])){
    ...
    if ($_POST['edit_id']){
        ...
        donation='".$_POST['edit_donation']."',amount='".$_POST['edit_amount']."',currency='".$_POST['edit_currency']."',realm='".$_POST['rid']."' WHERE id='".$_POST['edit_id']."'");
        ...
        echo "*In order to send gold as a reward, you must have Remote Acess setup and working!*<br /><br />
			  <form action='index.php?n=admin&sub=donate' method='POST'>
              Item: <input type='text' name='edit_items' value='".$row['items']."'>(Item ID. If more then 1 item, seperate with \",\"  Set to \"0\" For gold*)<br />
              Itemset: <input type='text' name='edit_itemset' value='".$row['itemset']."'>(Itemset ID, Leave blank if no Itemset)<br />
              Description: <input type='text' name='edit_description' value='".$row['description']."'><br />
			  Amount: <input type='text' name='edit_amount' value='".$row['amount']."'>  (How many of the item do they get. If gold, then amount is in copper)<br />
    ...
    if ($_POST['add_description'] != '' && $_POST['add_donation'] != '' && $_POST['add_currency'] != ''){
        ...
        (items,description,donation,amount,currency,itemset,realm) VALUES
        (
        '".$_POST['add_items']."',
...
<table>
	...
	<tr>
        <td><b>Realm name:</b> (Realm User Is In)</td>
        <td><select name="donate_realm"><?php echo $realmzlist ?></select></td>
    </tr>
...
<form action='index.php?n=admin&sub=donate' method='POST'>
*In order to send gold as a reward, you must have Remote Acess setup and working!*<br /><br />
<b>Item:</b> <input type='text' name='add_items'> (Item ID. If more then 1 item, seperate with ",". Set to "0" to Use gold*)<br />
<b>Itemset:</b> <input type='text' name='add_itemset'> (Itemset id. Leave Blank if no Itemset)<br />
<b>Description:</b> <input type='text' name='add_description'> (Little description or title)<br />
<b>Amount:</b> <input type='text' name='add_amount'> (How many of the item do they get. If gold, then amount is in copper)<br />
