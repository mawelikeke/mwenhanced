...
// ==================== //
  ...
  $cc = 0;
    ...
    {
        if($CHDB)$query = $CHDB->select("SELECT name, race, class, level, gender, zone  FROM `characters` WHERE `online`='1'  AND (`extra_flags` & 1 AND NOT `extra_flags` & 16) ORDER BY `name`");
    }else{
        output_message('alert','Realm <b>'.$realm_info['name'].'</b> is offline <img src="./templates/' . $offtmp . '/images/downarrow2.gif" border="0" align="top">');
    }
