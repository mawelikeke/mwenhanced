<?php
// Teamspeak Display Preview Release 4
// Copyright (C) 2005-2008  Guido van Biemen (aka MrGuide@NL)
//
...
class teamspeakDisplayClass {
	...
	function displayTeamspeakEx($settings) {
		$serverInfo = array_key_exists("result", $settings) ? $settings["result"] : $this->queryTeamspeakServerEx($settings);

		$result = "<div id=\"teamspeakdisplay\">\n";
		if ($serverInfo["queryerror"] != 0) {
			...
			$result .= "<img src=\"teamspeakdisplay/teamspeak_offline.png\" alt=\"\" title=\"" . $popupInfo . "\">";
			$result .= "<span class=\"teamspeakserver\" title=\"" . $popupInfo . "\">";
			...
			$result .= "function stringOk_" . $jsTeamspeakId . "(string, forbiddenChars) {\n";
			$result .= "	for(var i = 0; i < string.length; i++) {\n";
			$result .= "		if (forbiddenChars.indexOf(string.charAt(i)) > -1) {\n";
			$result .= "			return false;\n";
			$result .= "		}\n";
			$result .= "	}\n";
			$result .= "	return true;\n";
			$result .= "}\n";
			$result .= "function enterServer_" . $jsTeamspeakId . "() {\n";
			$result .= "	enterSubChannel_" . $jsTeamspeakId . "(null, false, null);\n";
			$result .= "}\n";
			$result .= "function enterChannel_" . $jsTeamspeakId . "(channelName, channelPassworded) {\n";
			$result .= "	enterSubChannel_" . $jsTeamspeakId . "(channelName, channelPassworded, null);\n";
			$result .= "}\n";
			$result .= "function enterSubChannel_" . $jsTeamspeakId . "(channelName, channelPassworded, subChannelName) {\n";
			$result .= "	var serveraddress = 'teamspeak://" . $settings["serveraddress"] . ":" . $settings["serverudpport"] . "';\n";
			$result .= "	var nickname=window.prompt('Enter your nickname', '');\n";
			$result .= "	if (nickname == null) {\n";
			$result .= "		return;\n";
			$result .= "	} else if (! stringOk_" . $jsTeamspeakId . "(nickname, '" . str_replace("'", "\\'", $settings["forbiddennicknamechars"]) . "')) {\n";
			$result .= "		window.alert('Could not enter the teamspeak server because the nickname you entered contains one or more of these forbidden characters: " . str_replace("'", "\\'", $settings["forbiddennicknamechars"]) . "');\n";
			$result .= "		return;\n";
			$result .= "	} else if (nickname == \"\") {\n";
			$result .= "		window.alert('Could not enter the teamspeak server because you did not enter your nickname');\n";
			$result .= "		return;\n";
			$result .= "	}\n";
			$result .= "	serveraddress = serveraddress + \"/nickname=\" + escape(nickname);\n";
			if ($serverInfo["serverinfo"]["server_password"] == "1") {
				$result .= "	var password=window.prompt('Enter the teamspeak server password for " . $serverInfo["serverinfo"]["server_name"] . "', '');\n";
				$result .= "	if (password == null) {\n";
				$result .= "		return;\n";
				$result .= "	} else if (password == \"\") {\n";
				$result .= "		window.alert('Could not enter the teamspeak server because you did not enter a server password');\n";
				$result .= "		return;\n";
				$result .= "	}\n";
				$result .= "	serveraddress = serveraddress + \"?password=\" + escape(password);\n";
			}
			$result .= "	if (channelName != null) { serveraddress = serveraddress + \"?channel=\" + escape(channelName); }\n";
			$result .= "	if (channelPassworded) {\n";
			$result .= "		var channelpassword=window.prompt('Enter the channel password for channel ' + channelName, '');\n";
			$result .= "		if (channelpassword == null) {\n";
			$result .= "			return;\n";
			$result .= "		} else if (channelpassword == \"\") {\n";
			$result .= "			window.alert('Could not enter the teamspeak server because you did not enter a channel password');\n";
			$result .= "			return;\n";
			$result .= "		}\n";
			$result .= "		serveraddress = serveraddress + \"?channelpassword=\" + escape(channelpassword);\n";
			$result .= "	}\n";
			$result .= "	if (subChannelName != null) { serveraddress = serveraddress + \"?subchannel=\" + escape(subChannelName); }\n";
			$result .= "	window.location=serveraddress;\n";
			$result .= "}\n";
			$result .= "//--></script>\n";
			...
			$result .= "<img src=\"teamspeakdisplay/teamspeak_online.png\" alt=\"\" title=\"" . $popupInfo . "\"/>";
			$result .= "<span class=\"teamspeakserver\" title=\"" . $popupInfo . "\">";
			$result .= "<a class=\"teamspeakserver\" href=\"javascript:enterServer_" . $jsTeamspeakId . "();\">";
			$result .= str_replace(" ", "&nbsp;", htmlspecialchars($serverInfo["serverinfo"]["server_name"]));
			$result .= "</a>";
			$result .= "</span><br/>\n";
			...
			foreach($serverInfo["channellist"] as $channelInfo) { if ($channelInfo["parent"] == -1) {
				...
				// Display channel:
				$result .= "<img src=\"teamspeakdisplay/treeimage" . ((($counter + 1) == $currentchannels) ? "3" : "2") . ".png\" alt=\"\"/>";
				$result .= "<img src=\"teamspeakdisplay/channel.png\" alt=\"\" title=\"" . $popupInfo . "\"/>";
				$result .= "<span class=\"teamspeakchannel\" title=\"" . $popupInfo . "\">";
				$result .= "<a class=\"teamspeakchannel\" href=\"javascript:enterChannel_" . $jsTeamspeakId . "('" . str_replace("'", "\\'", $channelInfo["channelname"]) . "', " . (($channelInfo["password"]) == "1" ? "true" : "false") . ");\">";
				$result .= str_replace(" ", "&nbsp;", htmlspecialchars($channelInfo["displayname"]));
				$result .= "</a>";
				$result .= "</span><br/>\n";
				...
				foreach($serverInfo["playerlist"] as $playerInfo) {
					...
					if ($playerInfo["channelid"] == $channelInfo["channelid"]) {
						...
						// Display player:
						$result .= "<img src=\"teamspeakdisplay/treeimage" . ((($counter + 1) == $currentchannels) ? "4" : "1") . ".png\" alt=\"\"/>";
						$result .= "<img src=\"teamspeakdisplay/treeimage" . ((($counter_playerandsubchannels + 1) == $currentplayersandsubchannels) ? "3" : "2") . ".png\" alt=\"\"/>";
						$result .= "<img src=\"teamspeakdisplay/player_" . $playerInfo["displayimage"] . ".png\" alt=\"" . $playerInfo["displayimage"] . "\" title=\"" . $popupInfo . "\"/>";
						$result .= "<span class=\"teamspeakplayer\" title=\"" . $popupInfo . "\">";
						$result .= str_replace(" ", "&nbsp;", htmlspecialchars($playerInfo["displayname"]));
						$result .= "</span><br/>\n";
				...
				foreach($serverInfo["channellist"] as $subchannelInfo) { if ($subchannelInfo["parent"] == $channelInfo["channelid"]) {
					...
					$result .= "<img src=\"teamspeakdisplay/treeimage" . ((($counter_playerandsubchannels + 1) == $currentplayersandsubchannels) ? "3" : "2") . ".png\" alt=\"\"/>";
					$result .= "<img src=\"teamspeakdisplay/channel.png\" alt=\"\" title=\"" . $popupInfo . "\"/>";
					$result .= "<span class=\"teamspeaksubchannel\" title=\"" . $popupInfo . "\">";
					$result .= "<a class=\"teamspeaksubchannel\" href=\"javascript:enterSubChannel_" . $jsTeamspeakId . "('" . str_replace("'", "\\'", $channelInfo["channelname"]) . "', " . (($channelInfo["password"]) == "1" ? "true" : "false") . ", '" . str_replace("'", "\\'", $subchannelInfo["channelname"]) . "');\">";
					$result .= str_replace(" ", "&nbsp;", htmlspecialchars($subchannelInfo["displayname"]));
					$result .= "</a>";
					$result .= "</span><br/>\n";
					...
					foreach($serverInfo["playerlist"] as $playerInfo) {
						...
						if ($playerInfo["channelid"] == $subchannelInfo["channelid"]) {
							...
							// Display player:
							$result .= "<img src=\"teamspeakdisplay/treeimage" . ((($counter + 1) == $currentchannels) ? "4" : "1") . ".png\" alt=\"\"/>";
							$result .= "<img src=\"teamspeakdisplay/treeimage" . ((($counter_playerandsubchannels + 1) == $currentplayersandsubchannels) ? "4" : "1") . ".png\" alt=\"\"/>";
							$result .= "<img src=\"teamspeakdisplay/treeimage" . ((($counter_player + 1) == $currentplayers) ? "3" : "2") . ".png\" alt=\"\"/>";
							$result .= "<img src=\"teamspeakdisplay/player_" . $playerInfo["displayimage"] . ".png\" alt=\"" . $playerInfo["displayimage"] . "\" title=\"" . $popupInfo . "\"/>";
							$result .= "<span class=\"teamspeakplayer\" title=\"" . $popupInfo . "\">";
							$result .= str_replace(" ", "&nbsp;", htmlspecialchars($playerInfo["displayname"]));
							$result .= "</span><br/>\n";
