...
$pbwow_config = obtain_pbwow_config(); // for PBWoW
$cfg_blizz_ranks = array(); // for PBWoW
$cfg_blizz_ranks = explode(',', $pbwow_config['pbwow_blizz_ranks']); // for PBWoW
$cfg_propass_ranks = array(); // for PBWoW
$cfg_propass_ranks = explode(',', $pbwow_config['pbwow_propass_ranks']); // for PBWoW
	...
	if (!isset($user_cache[$poster_id]))
	{
		...
		else
		{
			...
			$user_cache[$poster_id] = array(
				...
				'avatar'		=> ($user->optionget('viewavatars')) ? get_user_avatar($row['user_avatar'], $row['user_avatar_type'], $row['user_avatar_width'], $row['user_avatar_height']) : '',
				'avatar_src'				=> ($user->optionget('viewavatars')) ? get_user_avatar_src($row['user_avatar'], $row['user_avatar_type']) : '',  // for PBWoW
				'special_rank_title'		=> '', // for PBWoW
				'special_rank_image'		=> '', // for PBWoW
				'special_rank_image_src'	=> '', // for PBWoW
				'rank_blizz'				=> '', // for PBWoW
				'rank_propass'				=> '', // for PBWoW
				'age'			=> '',
			...
			{
				$user_cache[$poster_id]['rank_blizz'] = (in_array($row['user_rank'], $cfg_blizz_ranks)) ? true : false;
			}
			...
			{
				$user_cache[$poster_id]['rank_propass'] = (in_array($row['user_rank'], $cfg_propass_ranks)) ? true : false;
			}
...
for ($i = 0, $end = sizeof($post_list); $i < $end; ++$i)
{
	...
	$postrow = array(
		...
		'POST_NUMBER'			=> $post_number, // for PBWoW
		'POSTER_AVATAR_SRC'		=> $user_cache[$poster_id]['avatar_src'], // for PBWoW
		'SPECIAL_RANK_TITLE'	=> $user_cache[$poster_id]['special_rank_title'], // for PBWoW
		'SPECIAL_RANK_IMG'		=> $user_cache[$poster_id]['special_rank_image'], // for PBWoW
		'SPECIAL_RANK_IMG_SRC'	=> $user_cache[$poster_id]['special_rank_image_src'], // for PBWoW
		'RANK_BLIZZ'			=> $user_cache[$poster_id]['rank_blizz'], // for PBWoW
		'RANK_PROPASS'			=> $user_cache[$poster_id]['rank_propass'], // for PBWoW
