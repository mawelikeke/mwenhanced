...
$return_chars	= request_var('ch', ($topic_id) ? -1 : 180); // for PBWoW, default 300
...
*/
	...
	if ($sql_where)
	{
		if ($show_results == 'posts')
		{
			...
			//$sql = 'SELECT p.*, f.forum_id, f.forum_name, t.*, u.username, u.username_clean, u.user_sig, u.user_sig_bbcode_uid, u.user_colour // for PBWoW
		...
		else
		{
			...
			while ($row = $db->sql_fetchrow($result))
			{
				...
				if (!isset($user_cache[$row['poster_id']]))
				{
					...
					{
						$user_cache[$row['poster_id']] = array(
							'avatar_src'		=> ($user->optionget('viewavatars')) ? get_user_avatar_src($row['user_avatar'], $row['user_avatar_type']) : '',
							'rank_title'		=> '',
							'rank_image'		=> '',
							'rank_image_src'	=> '',
							'special_rank_title'		=> '',
				...
				{
					$bbcode_bitfield = $bbcode_bitfield | base64_decode($row['bbcode_bitfield']); // for PBWoW
					$row['post_text_full'] = $row['post_text']; // for PBWoW
		...
		foreach ($rowset as $row)
		{
			...
			if ($show_results == 'topics')
			{
				...
				$tpl_ary = array(
					...
					'TOPIC_AUTHOR_RANK_TITLE'	=> $row['topic_first_poster_rank_title'], // for PBWoW
					'TOPIC_AUTHOR_RANK_IMG'		=> (!empty($row['topic_first_poster_rank_img'])) ? '<img src="' . $phpbb_root_path . $config['ranks_path'] . '/' . $row['topic_first_poster_rank_img'] . '" border="0" alt="' . $row['topic_first_poster_rank_title'] . '" title="' . $row['topic_first_poster_rank_title'] . '"' : '', // for PBWoW
					'TOPIC_AUTHOR_RANK_IMG_SRC'	=> (!empty($row['topic_first_poster_rank_img'])) ? $phpbb_root_path . $config['ranks_path'] . '/' . $row['topic_first_poster_rank_img'] : '', // for PBWoW
			...
			else
			{
				...
				if ($row['display_text_only'])
				{
					...
					{
						$bbcode->bbcode_second_pass($row['post_text_full'], $row['bbcode_uid'], $row['bbcode_bitfield']);
					}
					$row['post_text_full'] = bbcode_nl2br($row['post_text_full']);
					$row['post_text_full'] = smiley_text($row['post_text_full']);
					// END needed for PBWoW
				...
				$tpl_ary = array(
					...
					'RANK_TITLE'		=> $user_cache[$row['poster_id']]['rank_title'], // for PBWoW
					'RANK_IMG'			=> $user_cache[$row['poster_id']]['rank_image'], // for PBWoW
					'RANK_IMG_SRC'		=> $user_cache[$row['poster_id']]['rank_image_src'], // for PBWoW
					'SPECIAL_RANK_TITLE'	=> $user_cache[$row['poster_id']]['special_rank_title'], // for PBWoW
					'SPECIAL_RANK_IMG'		=> $user_cache[$row['poster_id']]['special_rank_image'], // for PBWoW
					'SPECIAL_RANK_IMG_SRC'	=> $user_cache[$row['poster_id']]['special_rank_image_src'], // for PBWoW
					'RANK_BLIZZ'			=> $user_cache[$row['poster_id']]['rank_blizz'], // for PBWoW
					'RANK_PROPASS'			=> $user_cache[$row['poster_id']]['rank_propass'], // for PBWoW
					'POSTER_AVATAR_SRC'	=> $user_cache[$row['poster_id']]['avatar_src'], // for PBWoW
					'S_CUSTOM_FIELDS'	=> (isset($cp_row['row']) && sizeof($cp_row['row'])) ? true : false, // for PBWoW
					'MESSAGE_FULL'		=> $row['post_text_full'], // for PBWoW
