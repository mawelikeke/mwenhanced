...
class custom_profile
{
	...
	function generate_profile_fields_template($mode, $user_id = 0, $profile_row = false)
	{
		...
		else if ($mode == 'show')
		{
			...
			foreach ($profile_row as $ident => $ident_ary)
			{
				...
				$idx = $this->get_profile_id($ident_ary); // for PBWoW, by shArk
				...
				$tpl_fields['row'] += array(
					...
					'PROFILE_' . strtoupper($ident) . '_ID'		=> $idx, // for PBWoW, by shArk
				...
				$tpl_fields['blockrow'][] = array(
					...
					'PROFILE_FIELD_ID'		=> $idx, // for PBWoW, by shArk
	...
	*/
	function get_profile_value($ident_ary)
	{
		...
		$field_ident = $ident_ary['data']['field_ident']; // for PBWoW
		...
		switch ($this->profile_types[$field_type])
		{
			...
			case 'text':
				...
				if ($field_ident == ('pbarmorycharlink' || 'pbarmoryguildlink')) // for PBWoW
				{
					// Do nothing
				}
				else
	...
	/**
	* Get Profile Id for display for PBWoW, by shArk
	*/
	function get_profile_id($ident_ary)
	{
	   $value = $ident_ary['value'];
	   $field_type = $ident_ary['data']['field_type'];
	   $value = $value - 1;
