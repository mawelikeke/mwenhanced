...
function display_forums($root_data = '', $display_moderators = true, $return_moderators = false)
{
	...
	foreach ($forum_rows as $row)
	{
		...
		{
		// START needed for PBWoW, original by Missparker, modified by PayBas
			$cat_unread = false;
			foreach ($forum_rows as $frows)
			{
				if ($frows['parent_id'] == $row['forum_id'])
				{
					...
					if (isset($subforums[$frows['forum_id']]))
					{
						foreach ($subforums[$frows['forum_id']] as $subforum_id => $subforum_row)
						{
							$subforum_unread = (isset($forum_tracking_info[$subforum_id]) && $subforum_row['orig_forum_last_post_time'] > $forum_tracking_info[$subforum_id]) ? true : false;
							if ($subforum_unread) break;
						}
					}
			...
			// END needed for PBWoW, original by Missparker, modified by PayBas
			$template->assign_block_vars('forumrow', array(
			'S_NO_CAT_LAST'			=> $last_catless, // for PBWoW
				'S_UNREAD_FORUM'		=> $cat_unread, // for PBWoW, by Missparker
				'S_IS_CAT'				=> true,
				'FORUM_ID'				=> $row['forum_id'],
		...
		$template->assign_block_vars('forumrow', array(
			'S_NO_CAT_EACH'		=> $catless, // for PBWoW
			'S_IS_CAT'			=> false,
